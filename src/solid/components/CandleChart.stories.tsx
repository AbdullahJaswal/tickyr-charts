...
export const TooltipCustom: Story = {
  args: {
    tooltip: (props: {
      ...
      idx: number
      c: number
      formatter: { formatPrice: (n: number) => string }
    }) => (
      <div
        style={{
          position: "absolute",
          left: `${props.pointerX + 12}px`,
          top: `${props.pointerY + 12}px`,
...
function LiveTickHarness(): JSX.Element {
  ...
  createEffect(() => {
    ...
    const tick = (): void => {
      ...
      state.price = Math.max(50, state.price + drift)
      const dtMs = 200 + Math.floor(u * 400)
      state.nextTs = state.nextTs + dtMs
      const priceRaw = Math.max(1, Math.round(state.price * PRICE_SCALE))
      const volRaw = 10 + Math.floor(u * 90)
      try {
        pushTick(state.nextTs, priceRaw, volRaw)
      } catch {
...
export const PositionWithTooltip: Story = {
  args: {
    ...
    positionTooltip: (p: {
      marker: { qty: number; entryPrice: number }
      lastClose: number
      formatter: { formatPrice: (n: number) => string }
    }) => (
      <div
...
export const EventWithTooltip: Story = {
  args: {
    ...
    eventTooltip: (p: { marker: { kind: string; title: string } }) => (
      <div
        style={{
          background: "rgba(0,0,0,0.85)",
          color: "#fff",
