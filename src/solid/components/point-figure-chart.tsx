...
// Solid PointFigureChart adapter.
...
export type PointFigureChartTooltipProp =
  | undefined
  | false
  | ((props: PnFChartTooltipProps) => JSX.Element)
...
export function PointFigureChart(props: PointFigureChartProps): JSX.Element {
  ...
  let controller: PnFChartController | null = null
  const [renderCtx, setRenderCtx] = createSignal<PnFChartRenderContext | null>(
    null,
  )
  const [hover, setHover] = createSignal<HoverState | null>(null)

  const providerSnapshot = createMemo<PnFChartProviderSnapshot>(() => {
  ...
  onMount(() => {
    ...
    controller = new PnFChartController({
...
function defaultTooltip(p: PnFChartTooltipProps): JSX.Element {
