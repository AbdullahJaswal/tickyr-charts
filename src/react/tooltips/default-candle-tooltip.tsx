import * as React from "react"

import type { Palette, Theme } from "../../personalization"
import type {
  CandleChartTooltipProps,
  CandleDirection,
} from "../../charts/candle-chart-helpers"

// `CandleChartTooltipProps` and `CandleDirection` are framework-agnostic
// data shapes - they live in `charts/candle-chart-helpers.ts`. Re-export
// here so existing import paths keep resolving.
export type { CandleChartTooltipProps, CandleDirection }

const TOOLTIP_OFFSET = 12
const TOOLTIP_WIDTH_HINT = 180
const TOOLTIP_HEIGHT_HINT = 124

interface TooltipPalette {
  bg: string
  fg: string
  subtleFg: string
  border: string
  shadow: string
  upFg: string
  downFg: string
}

function getTooltipPalette(theme: Theme, palette: Palette): TooltipPalette {
  // We don't pull oklch→css here to avoid a circular import; the consuming
  // CandleChart resolves direction colors at the call-site already. Defaults
  // for direction text use a stable green/red pair so the OHLC numbers
  // remain readable on either palette.
  void palette
  if (theme === "light") {
    return {
      bg: "rgb(255, 255, 255)",
      fg: "rgb(20, 22, 26)",
      subtleFg: "rgba(20, 22, 26, 0.56)",
      border: "rgba(15, 18, 23, 0.08)",
      shadow:
        "0 8px 24px rgba(15, 18, 23, 0.10), 0 2px 6px rgba(15, 18, 23, 0.06)",
      upFg: "rgb(20, 22, 26)",
      downFg: "rgb(20, 22, 26)",
    }
  }
  return {
    bg: "rgb(34, 36, 42)",
    fg: "rgb(240, 242, 246)",
    subtleFg: "rgba(240, 242, 246, 0.62)",
    border: "rgba(255, 255, 255, 0.08)",
    shadow: "0 8px 24px rgba(0, 0, 0, 0.55), 0 2px 6px rgba(0, 0, 0, 0.40)",
    upFg: "rgb(240, 242, 246)",
    downFg: "rgb(240, 242, 246)",
  }
}

export function DefaultCandleTooltip(
  props: CandleChartTooltipProps,
): React.ReactElement {
  const {
    t,
    o,
    h,
    l,
    c,
    direction,
    pointerX,
    pointerY,
    containerWidth,
    containerHeight,
    theme,
    palette,
    formatter,
  } = props
  const colors = getTooltipPalette(theme, palette)

  const showLeft = pointerX > containerWidth - TOOLTIP_WIDTH_HINT
  const showAbove = pointerY > containerHeight - TOOLTIP_HEIGHT_HINT
  const positional: React.CSSProperties = showLeft
    ? { right: `${containerWidth - pointerX + TOOLTIP_OFFSET}px` }
    : { left: `${pointerX + TOOLTIP_OFFSET}px` }
  if (showAbove) {
    positional.bottom = `${containerHeight - pointerY + TOOLTIP_OFFSET}px`
  } else {
    positional.top = `${pointerY + TOOLTIP_OFFSET}px`
  }

  const change = c - o
  const changeAbs = Math.abs(change)
  const changePct = o !== 0 ? (change / o) * 100 : 0
  const directionFg =
    direction === "up"
      ? colors.upFg
      : direction === "down"
        ? colors.downFg
        : colors.subtleFg
  const sign = change > 0 ? "+" : change < 0 ? "−" : ""

  return (
    <div
      style={{
        position: "absolute",
        ...positional,
        pointerEvents: "none",
        background: colors.bg,
        color: colors.fg,
        padding: "8px 11px",
        borderRadius: 6,
        border: `1px solid ${colors.border}`,
        fontSize: 12,
        fontFamily: "system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif",
        lineHeight: 1.35,
        whiteSpace: "nowrap",
        boxShadow: colors.shadow,
        zIndex: 1,
        minWidth: 160,
      }}
      role="status"
      aria-live="polite"
    >
      <div
        style={{
          color: colors.subtleFg,
          fontSize: 10.5,
          letterSpacing: "0.02em",
          textTransform: "uppercase",
          fontWeight: 500,
          marginBottom: 4,
        }}
      >
        {formatter.formatDate(t)} {formatter.formatTime(t)}
      </div>
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "auto 1fr",
          columnGap: 12,
          rowGap: 2,
          fontVariantNumeric: "tabular-nums",
        }}
      >
        <span style={{ color: colors.subtleFg, fontWeight: 500 }}>O</span>
        <span style={{ textAlign: "right", fontWeight: 600 }}>
          {formatter.formatPrice(o)}
        </span>
        <span style={{ color: colors.subtleFg, fontWeight: 500 }}>H</span>
        <span style={{ textAlign: "right", fontWeight: 600 }}>
          {formatter.formatPrice(h)}
        </span>
        <span style={{ color: colors.subtleFg, fontWeight: 500 }}>L</span>
        <span style={{ textAlign: "right", fontWeight: 600 }}>
          {formatter.formatPrice(l)}
        </span>
        <span style={{ color: colors.subtleFg, fontWeight: 500 }}>C</span>
        <span
          style={{ textAlign: "right", fontWeight: 600, color: directionFg }}
        >
          {formatter.formatPrice(c)}
        </span>
      </div>
      <div
        style={{
          marginTop: 4,
          paddingTop: 4,
          borderTop: `1px solid ${colors.border}`,
          color: colors.subtleFg,
          fontWeight: 500,
          display: "flex",
          justifyContent: "space-between",
          gap: 12,
          fontVariantNumeric: "tabular-nums",
        }}
      >
        <span>Δ</span>
        <span style={{ color: directionFg, fontWeight: 600 }}>
          {sign}
          {formatter.formatPrice(changeAbs)} ({sign}
          {Math.abs(changePct).toFixed(2)}%)
        </span>
      </div>
    </div>
  )
}
